export type HeroTemplates =
  | "simple"
  | "main"
  | "half"
  | "marquee"
  | "only-dark"
  | "desaturate"
  | "big"
  | "double-marquee"
  | "split"
  | "space"
  | "management"
  | "care"
  | "ivvi"
  | "luxury"
  | 'earth'
  | 'next'
  | 'coffee'
  | 'high-converting';
export type HeadersTemplates =
  | "simple"
  | "saas"
  | "indeed"
  | "float"
  | "multi"
  | "store"
  | "agency"
export type FeaturesTemplates =
  | "elegance"
  | "puzzle"
  | "explore"
  | "central"
  | "mush"
  | "flex"
  | "trebol"
  | "portrait"
  | "classic"
  | "ancient"
  | "bento"
  | "luxury"
  | "high-converting";
export type FootersTemplates =
  | "simple"
  | "space"
  | "full"
  | "big"
  | "minimal"
  | "openai"
  | "gradient";
export type CTATemplates = "simple" | 'social' | 'serif' | 'art' | 'solid' | 'gradient' | 'app' | 'connect' | 'saturate' | 'overflow' | 'sparkles' | 'high-converting' | 'heading'
export type TestimonialsTemplates = 'masonry' | 'pastelide' | 'tiles' | 'stars' | 'bento' | 'high-converting'
export type ContactTemplates = 'simple' | 'single' | 'enterprise' | 'map' | 'multi'
export type LogosTemplates = 'simple' | 'animated' | 'multi' | 'mini' | 'double' | 'high-converting'
export type FAQTemplates = 'simple' | 'border' | 'grid' | 'high-converting' 
export type AuthTemplates = 'simple' | 'logo' | 'big' | 'create' | 'reset'
export type GalleryTemplates = 'grid' | 'windows' | 'carousel' | 'lightbox' | 'minimal' | 'modern'
