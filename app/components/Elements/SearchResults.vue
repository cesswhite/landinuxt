  ...
  <div class="w-full px-2 py-4">
    ...
    <section v-if="sections.length" class="mb-10">
      ...
      <div class="grid w-full grid-cols-12 items-start gap-4">
        <ElementsTile v-for="section in sections" :key="section.name" :data="section" />
      </div>
    </section>
    ...
    <section v-if="components.length">
      ...
      <ul class="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <li v-for="item in components" :key="`${item.parent}-${item.title}`">
          <NuxtLink :to="componentDetailPath(item)"
            ...
            <ElementsSearchPreview :parent="item.parent" :component="item.component" :title="item.title"
            ...
            <div class="flex min-w-0 flex-1 flex-col">
              ...
              <div v-if="item.tags.length" class="mt-3 flex flex-wrap gap-1.5">
                <UBadge v-for="tag in item.tags.slice(0, 5)" :key="tag" color="neutral" variant="outline" size="sm">
                  {{ tag }}
                </UBadge>
              </div>
            </div>
    ...
    <div v-if="!sections.length && !components.length"
