                ...
                <template v-for="(col, index) in testimonials" :key="index">
                    <div class="h-full col-span-full md:col-span-4 flex flex-col gap-4 even:mt-4">
                        <template v-for="(testimonial, _index) in col" :key="_index">
                            <div
                                class="rounded-2xl border border-dark-200 p-4 bg-dark-100 hover:bg-dark-200/70 dark:bg-dark-800/50 dark:border-dark-800 dark:hover:bg-dark-800/70 transition-all duration-300 ease-in-out">
                                <p class="font-normal text-base text-dark-950/80 dark:text-dark-50/60">
                                    {{ testimonial.text }}
                                </p>
                                <div class="mt-20">
                                    <span
                                        class="inline-block w-full font-bold text-sm text-dark-950 dark:text-dark-50">{{
                                            testimonial.name }}</span>
                                    <span
                                        class="inline-block w-full font-normal text-sm text-dark-950/60 dark:text-dark-50/60">{{
